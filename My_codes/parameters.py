class Parameters:
    ...
    def __init__(self,
        ...
        if method in ['choco', 'new']:
            assert consensus_lr > 0
        else:
            assert consensus_lr is None

        assert quantization in ['full', 'top', 'random-biased', 'random-unbiased',
                                'qsgd-biased', 'qsgd-unbiased', 'top-qsgd-unbiased', 'random-qsgd-unbiased', 'new-qsgd-unbiased', "new-qsgd-biased"]
        ...
        assert topology in ['centralized', 'ring', 'torus', 'disconnected', 'star', 'random']

        assert method in ['choco', 'dcd-psgd', 'ecd-psgd', 'plain', 'new']
        if method in ['dcd-psgd', 'ecd-psgd']:
            assert quantization in ['top','random-unbiased', 'qsgd-unbiased','top-qsgd-unbiased', 'random-qsgd-unbiased']
        if method  in ['new']:
